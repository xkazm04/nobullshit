...
const ChallengeAiRecommend = ({habit, user}:Props) => {
    ...
    <div className="flex flex-col gap-2 justify-start">
        {recommendation && recommendation.map((item, index) => (
            <div key={index} className="flex justify-start text-left">
                <div className="bg-gray-950 p-2 flex flex-col justify-between relative rounded-xl items-start text-sm w-full min-w-[350px]">
                    <div>{item.name}</div> 
